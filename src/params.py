...
class CosmoBase(object):
    ...
    def __init__(self, 
        ...
        w0, wa : float, optional
            Dark energy equation of state parameters. Defaults: w0, wa = (-1, 0)
    ...
    def validate(self):
        ...
        if self.sigma_8 is not None: assert(self.sigma_8 >= 0.)
        
        # Density parameters: Consistency relations
        assert(self.Omega_m == (self.Omega_b + self.Omega_c + self.Omega_n_mass))
...
if __name__ == '__main__':
    c = CosmoBase(Omega_c=0.27, Omega_b=0.05, Omega_l=0.69, h=0.67, 
                  n_s=0.96, A_s=1e-9)
    c.validate()
    print c
